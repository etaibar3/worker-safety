...
router.post('/', authenticateUser, async function(req, res){
    ...
    try{
        ...
        for (const record of result1.records) {
          const reserve_date = await new Date(result2.records[index]._fields[0].properties.time);
          if(reserve_date <= today && reserve_date >= reported) {
            const { email } = await User.findById(record._fields[0].properties.m_id)
            const alreadyAdded = await exposed_persons.includes(email);
            if(!alreadyAdded) {
